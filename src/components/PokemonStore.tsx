...
export const PokemonStore: React.FC<PokemonStoreProps> = ({ storePokemon, player, onPurchase }) => {
  return (
        ...
        {storePokemon.map((pokemon) => (
          <div key={pokemon.id} className="relative">
            ...
            <div className="mt-2 text-center">
              <p className="text-lg font-semibold text-green-600">{pokemon.cost} points</p>
              <button
                onClick={() => onPurchase(pokemon)}
                disabled={player.points < (pokemon.cost || 0) || player.inventory.length >= 12}
                className={`mt-2 px-4 py-2 rounded-lg w-full ${
                  player.points >= (pokemon.cost || 0) && player.inventory.length < 12
